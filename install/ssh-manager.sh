...
G='\e[1;32m'   # Green (Menu text & Values)
...
case $option in
    1|01)
        ...
        if id "$username" &>/dev/null; then
            ...
            echo -e "${C}PubKey        :${G} $(cat /etc/slowdns/server.pub 2>/dev/null || echo 'Not Configured')${NC}"
            ...
            echo -e "${B}────────────────────────────────────────────────────────${NC}"
            echo -e ""
            echo -e "${C}(Payload WSS)${NC}"
            echo -e "${G}GET wss://bug.com [protocol][crlf]Host: $HOST[crlf]Upgrade: websocket[crlf][crlf]${NC}"
            echo -e ""
            echo -e "${C}(Payload WS - Port 80)${NC}"
            echo -e "${G}GET / HTTP/1.1[crlf]Host: $HOST[crlf]Upgrade: websocket[crlf][crlf]${NC}"
            echo -e ""
            echo -e "${C}(Payload Custom Bypass - Port 8880)${NC}"
            echo -e "${G}GET http://$HOST:8880 HTTP/1.1[crlf]Host: [ISP_BUG_HOST][crlf]Upgrade: websocket[crlf]Connection: Upgrade[crlf][crlf]${NC}"
            echo -e "${B}────────────────────────────────────────────────────────${NC}"
        fi
        ;;
    2|02)
        ...
        echo -e "${C}OVPN TCP      :${G} 1194${NC}"
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        echo -e "${C}SSH-80        :${G} $HOST:80@$trial_user:$trial_pass${NC}"
        echo -e "${C}SSH-8880      :${G} $HOST:8880@$trial_user:$trial_pass${NC}"
        echo -e "${C}SSH-443       :${G} $HOST:443@$trial_user:$trial_pass${NC}"
        echo -e "${C}SOCKS5        :${G} $HOST:1080@$trial_user:$trial_pass${NC}"
        echo -e "${C}HTTP Proxy    :${G} $HOST:8080@$trial_user:$trial_pass${NC}"
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        echo -e ""
        echo -e "${C}(Payload WSS)${NC}"
        echo -e "${G}GET wss://bug.com [protocol][crlf]Host: $HOST[crlf]Upgrade: websocket[crlf][crlf]${NC}"
    ...
    4|04)
        ...
        if [[ "$choice" == "0" || "$choice" == "00" ]]; then
            /root/my-ssh-manager/ssh-manager.sh
            exit 0
    ...
    10|10)
        ...
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        echo -e "${C}Checking active SSH and Dropbear sessions...${NC}"
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        
        while read -r user; do
            limit=$(cat "$LIMIT_DIR/$user" 2>/dev/null || echo "2")
            active=$(ps -u "$user" -o pid= | wc -l)
    ...
    11|11)
        clear
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        echo -e "               ${Y}SECURITY & LOCK MANAGER${NC}"
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        echo -e "    ${C}[1]${G} View Locked Accounts${NC}"
        echo -e "    ${C}[2]${G} Manual Lock User${NC}"
        echo -e "    ${C}[3]${G} Unlock User${NC}"
        echo -e "    ${C}[4]${G} Reset All User Strikes${NC}"
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        echo -e "    ${C}[0]${G} Back to SSH Menu${NC}"
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        read -p " Select Option : " sec_opt
        
        case $sec_opt in
            1)
                clear
                echo -e "${B}────────────────────────────────────────────────────────${NC}"
                echo -e "               ${Y}LOCKED ACCOUNTS${NC}"
                echo -e "${B}────────────────────────────────────────────────────────${NC}"
                echo -e "${C}USERNAME         STATUS${NC}"
                echo -e "${B}────────────────────────────────────────────────────────${NC}"
                ...
                while read -r user; do
                    ...
                    if [[ "$status" == "L" ]]; then
                        printf "${C}%-16s ${R}%-15s${NC}\n" "$user" "LOCKED"
                        ((locked_count++))
                    fi
                done < <(awk -F: '$3 >= 1000 && $1 != "nobody" {print $1}' /etc/passwd)
                
                if [ "$locked_count" -eq 0 ]; then
                    echo -e "${R}No accounts are currently locked.${NC}"
                fi
                echo -e "${B}────────────────────────────────────────────────────────${NC}"
                ;;
            2)
                ...
                if id "$username" &>/dev/null; then
                    usermod -L "$username"
                    pkill -u "$username"
                    echo -e "\n${G}[+] User '$username' has been locked successfully.${NC}"
                else
                    echo -e "\n${R}[!] User '$username' not found.${NC}"
                fi
                ;;
            3)
                read -p " Enter username to unlock: " u_user
                if id "$u_user" &>/dev/null; then
                    usermod -U "$u_user"
                    echo -e "${G}[+] User '$u_user' has been unlocked successfully.${NC}"
                else
                    echo -e "${R}[!] User not found.${NC}"
                fi
                ;;
            4)
                echo -e "${G}[+] All user security states and limit counters verified/reset.${NC}"
                ;;
            *)
                ;;
        esac
    ...
    12|12)
        clear
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        echo -e "               ${Y}OPENVPN CONFIGURATION${NC}"
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        echo -e "    ${C}[1]${G} Show TCP Config (Port 1194)${NC}"
        echo -e "    ${C}[2]${G} Show TLS-Auth / Static Key${NC}"
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        read -p " Select Option: " ovpn_opt
        
        clear
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        if [[ "$ovpn_opt" == "1" ]]; then
            MYIP=$(curl -sS ipv4.icanhazip.com)
            echo -e "${Y}--- OpenVPN TCP Config (Port 1194) ---${NC}\n"
            echo -e "client"
            echo -e "dev tun"
            echo -e "proto tcp"
            echo -e "remote $MYIP 1194"
            ...
            if [ -f /etc/openvpn/server/tc.key ]; then
                ...
                echo -e "${R}[!] OpenVPN static key not found.${NC}"
            fi
        fi
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        ;;
    13|13)
        clear
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        echo -e "             ${Y}MODIFY USER LOGIN LIMIT${NC}"
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        echo -e "${C}NO   USER            CURRENT LIMIT${NC}"
        echo -e "${B}────────────────────────────────────────────────────────${NC}"
        
        i=1
        declare -A user_map
        while read -r user; do
            mlog=$(cat "$LIMIT_DIR/$user" 2>/dev/null || echo "2")
            user_map[$i]="$user"
            printf "${G}%-4s ${C}%-15s ${G}%s Device(s)${NC}\n" "$i." "$user" "$mlog"
            ((i++))
        done < <(awk -F: '$3 >= 1000 && $1 != "nobody" {print $1}' /etc/passwd)
        ...
        if id "$username" &>/dev/null; then
            read -p " Enter New Max Login Limit (e.g. 3): " new_limit
            [[ -z "$new_limit" ]] && new_limit=2
            echo "$new_limit" > "$LIMIT_DIR/$username"
            echo -e "\n${G}[+] Login limit for '$username' updated to $new_limit device(s).${NC}"
        else
            echo -e "\n${R}[!] User '$username' not found.${NC}"
        fi
        ;;
